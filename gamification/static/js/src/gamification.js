    ...
    function post_success(result){
      ...
      if(mech_id == 0){
        if(mech_type == "Adaptative" && mech_size == "Widget"){
          console.log("https://agmodule.herokuapp.com/api/g_mechanics/" + adaptative_mech_id + "/?user=" + uname + "&dynamic_index=" + usage_id + "&dynamic_progress=" + progress + "&dynamic_link_url=" + dashboard_url);
          fetch("https://agmodule.herokuapp.com/api/g_mechanics/" + adaptative_mech_id + "/?user=" + uname + "&dynamic_index=" + usage_id + "&dynamic_progress=" + progress + "&dynamic_link_url=" + dashboard_url) //&dynamic_index={self.scope_ids.def_id} 
          ...
          .then(mech_list => (ids_list = [],
                    ...
                    ids_list))
          .then(ids_list => ( fetch("https://agmodule.herokuapp.com/api/g_mechanics/" + ids_list[Math.floor(Math.random() * ids_list.length)] + "/?user=" + uname + "&dynamic_index=" + usage_id + "&dynamic_progress=" + progress + "&dynamic_link_url=" + dashboard_url) //&dynamic_index={self.scope_ids.def_id} 
                    .then(response => response.json())
